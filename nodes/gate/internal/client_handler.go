...
func (ch *ClientHandler) Register(client *net.Client) {
	...
	client.SetHandler(pb.MsgID_FWM_GAME_NODE_LIST_ACK, ch.OnGameNodeListAck)
}
...
func (ch *ClientHandler) OnGameNodeListAck(message interface{}, agent interface{}) {
	msg := message.(*pb.GameNodeListAck)
	//a := agent.(*net.BaseAgent)

	for _, agentInfo := range msg.NodeInfos {
		if !ch.Node.ClientManager.ExistClient(agentInfo.Addr) {
			...
			ch.Node.ClientManager.NewAndStart(agentInfo.Addr, &ClientHandler{Node: ch.Node}, net.NewProcessor())
		}
	}
}
